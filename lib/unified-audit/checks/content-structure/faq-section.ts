...
import { CheckCategory, ScoreType } from '@/lib/unified-audit/types'

/**
 * Deduplicated from:
 * - lib/audit/checks/ai/no-faq-content.ts (site audit version)
 * - lib/aio/checks/content-structure/faq-section.ts (AIO version, richer)
 *
 * Takes the AIO version as the base (checks FAQ schema, headings, definition lists,
 * details/summary elements) and merges the site audit's class/id selector detection.
 */
export const faqSection: AuditCheckDefinition = {
  name: 'faq_section',
  ...
  feedsScores: [ScoreType.SEO, ScoreType.AIReadiness],

  async run(context: CheckContext): Promise<CheckResult> {
    const $ = cheerio.load(context.html)

    // Check for FAQPage schema (strict matching from AIO version)
    const hasFAQSchema =
    ...
    // Check for FAQ-related class/id attributes (from site audit version)
    const faqElements = $('[class*="faq"], [id*="faq"]').length
    ...
    if (faqElements > 0) faqIndicators.push('FAQ section element')
