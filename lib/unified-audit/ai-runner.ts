...
export async function runAIAnalysisPhase(
  ...
  if (!aiAnalysisEnabled) {
    console.log('[AI Runner] Skipping — AI analysis disabled for this audit')
    return {
      strategicScore: null,
      pagesAnalyzed: 0,
      totalInputTokens: 0,
  ...
  for (const pageImportance of topPages) {
    try {
      ...
      console.error(`[AI Runner] Failed to fetch ${pageImportance.url}:`, error)
    }
  }

  if (pageContents.length === 0) {
    console.log('[AI Runner] No pages could be fetched for analysis')
    return {
      strategicScore: null,
      pagesAnalyzed: 0,
      totalInputTokens: 0,
  ...
  if (aiAnalysisRows.length > 0) {
    ...
    if (error) {
      console.error('[AI Runner] Failed to insert AI analyses:', error)
    }
  }
  ...
  console.log(
    `[AI Runner] Completed: ${batchResult.analyses.length} pages, strategic score=${strategicScore}, cost=$${batchResult.totalCost.toFixed(4)}`
  )
