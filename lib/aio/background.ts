...
export async function runAIOAuditBackground(auditId: string, url: string): Promise<void> {
  ...
  try {
    ...
    const { checks, technicalScore } = await runAIOChecks({
      auditId,
      url,
      sampleSize: audit.sample_size ?? 5,
      async onCheckComplete(check: AIOCheck) {
