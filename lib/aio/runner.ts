 ...
 */
export async function runAIOChecks(options: AIORunnerOptions): Promise<AIORunnerResult> {
  const { auditId, url, onCheckComplete } = options
  const checks: AIOCheck[] = []

  console.log(`[AIO Runner] Starting checks for ${url}`)
  ...
  console.log(`[AIO Runner] Completed ${checks.length} checks, score: ${technicalScore}/100`)
 ...
 * - failed: 0 points
 */
function calculateTechnicalScore(checks: AIOCheck[]): number {
  let totalWeight = 0
  let weightedScore = 0
 ...
 */
export function getCategoryScores(checks: AIOCheck[]): {
