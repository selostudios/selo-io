import type { GEOCheckDefinition } from '@/lib/geo/types'
...
export const allGEOChecks: GEOCheckDefinition[] = [
...
export function getChecksByCategory(
  ...
  return allGEOChecks.filter((check) => check.category === category)
}

// Site-wide checks run once per audit (not per page)
export const siteWideChecks = allGEOChecks.filter((check) => check.isSiteWide)

// Page-specific checks run on each crawled page
export const pageSpecificChecks = allGEOChecks.filter((check) => !check.isSiteWide)
