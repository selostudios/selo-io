...
import { runGEOChecks, getCategoryScores } from '@/lib/geo/runner'
...
describe('GEO Runner', () => {
  ...
  describe('runGEOChecks', () => {
    it('should run all checks and calculate technical score', async () => {
      ...
      const result = await runGEOChecks({
        auditId: 'test-audit',
        url: 'https://example.com',
        sampleSize: 1,
      })
    ...
    it('should call onCheckComplete callback for each check', async () => {
      ...
      // Each call should receive a GEOCheck object
      const firstCall = onCheckComplete.mock.calls[0][0] as GEOCheck
