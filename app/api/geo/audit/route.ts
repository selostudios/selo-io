...
import { runGEOChecks } from '@/lib/geo/runner'
import { runAIAnalysis, calculateStrategicScore } from '@/lib/geo/ai-auditor'
import { selectTopPages } from '@/lib/geo/importance'
import { crawlSite } from '@/lib/audit/crawler'
import type { GEOCheck } from '@/lib/geo/types'
...
/**
 * POST /api/geo/audit
 * Creates and runs a GEO audit with Server-Sent Events (SSE) streaming
 */
...
/**
 * Process GEO audit in background with SSE streaming
 */
async function processAudit(
  ...
  try {
    ...
    await supabase.from('geo_audits').update({ status: 'running', started_at: new Date().toISOString() }).eq('id', auditId)
    ...
    const programmaticResult = await runGEOChecks({
      auditId,
      url,
      sampleSize,
      onCheckComplete: async (check: GEOCheck) => {
