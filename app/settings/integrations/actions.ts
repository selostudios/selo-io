...
export async function connectPlatform(formData: FormData) {
  ...
  if (!platform_type || !validPlatforms.includes(platform_type as any)) {
  ...
  if (error) {
    console.error('[Connect Platform Error]', { type: 'database_error', timestamp: new Date().toISOString() })
...
export async function disconnectPlatform(connectionId: string) {
  ...
  if (!userRecord || userRecord.role !== 'admin') {
    console.error('[Disconnect Platform Error]', { type: 'unauthorized', userId: user.id, timestamp: new Date().toISOString() })
  ...
  if (error) {
    console.error('[Disconnect Platform Error]', { type: 'database_error', timestamp: new Date().toISOString() })
