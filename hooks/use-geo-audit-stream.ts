...
export interface GEOAuditState {
...
export function useGEOAuditStream() {
  const [state, setState] = useState<GEOAuditState>({
    status: 'idle',
    auditId: null,
    pagesFound: 0,
    programmaticChecks: [],
  ...
  const handleEvent = useCallback((event: { type: string; [key: string]: unknown }) => {
    switch (event.type) {
      case 'status':
        setState((prev) => ({
          ...prev,
          status: event.status as GEOAuditState['status'],
        }))
        break
      ...
      case 'complete':
        setState((prev) => ({
          ...
          tokenUsage: event.tokenUsage as GEOAuditState['tokenUsage'],
      ...
      default:
        console.warn('[GEO Audit Stream] Unknown event type:', event.type)
    }
  }, [])
  ...
  const startAudit = useCallback(
    async (organizationId: string | null, url: string, sampleSize: number) => {
      ...
      try {
        ...
        console.error('[GEO Audit Stream] Error:', error)
        setState((prev) => ({
          ...prev,
          status: 'error',
