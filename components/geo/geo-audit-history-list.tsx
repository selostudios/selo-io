...
interface GEOAuditHistoryListProps {
  audits: GEOAudit[]
  showUrl?: boolean
}
...
function isInProgress(status: GEOAudit['status']): boolean {
...
export function GEOAuditHistoryList({ audits, showUrl = false }: GEOAuditHistoryListProps) {
  const router = useRouter()

  const handleDeleteAudit = async (auditId: string) => {
    try {
      const response = await fetch(`/api/geo/audit/${auditId}`, { method: 'DELETE' })
      if (response.ok) {
        router.refresh()
      }
    } catch (err) {
      console.error('[GEO Audit History] Failed to delete audit:', err)
    }
  }
  ...
  if (audits.length === 0) {
    return (
      <EmptyState
        icon={Sparkles}
        title="No GEO audits yet"
        description="Run your first GEO audit to analyze your content for AI search engines."
      />
    )
  }
          ...
          <div className="flex items-center gap-6">
            ...
            {isInProgress(audit.status) ? (
              ...
              <span className="font-medium tabular-nums">
                {audit.overall_geo_score !== null ? `${audit.overall_geo_score}/100` : '-'}
              </span>
            )}
